...
在这个导学里面，你将学习如何创建storm拓扑结构和发布

## Preliminaries

This tutorial uses examples from the [storm-starter](http://github.com/apache/storm/blob/v2.2.0/examples/storm-starter) project. It's recommended that you clone the project and follow along with the examples. Read [Setting up a development environment](http://storm.apache.org/releases/2.2.0/Setting-up-development-environment.html) and [Creating a new Storm project](http://storm.apache.org/releases/2.2.0/Creating-a-new-Storm-project.html) to get your machine set up.
...
storm集群和hadoop集群很相似。在hadoop里面使用 MapReduce job，在Storm里面使用topologies。jobs 和 topologies有很多不同的地方：job最终是会运行终止的，而topology会一直运行。
...
在storm集群里面有两种节点，一种是master node 一种是 worker node。master node主要责任是安排任务给集群中的节点，检测成功和失败。
...
![Storm cluster](http://storm.apache.org/releases/2.2.0/images/storm-cluster.png)
...
Since topology definitions are just Thrift structs, and Nimbus is a Thrift service, you can create and submit topologies using any programming language. The above example is the easiest way to do it from a JVM-based language. See [Running topologies on a production cluster](http://storm.apache.org/releases/2.2.0/Running-topologies-on-a-production-cluster.html)] for more information on starting and stopping topologies.
...
在strom中提供stream处理的事spout和bolts。在代码里面我们需要实现他们的接口。
...
![A Storm topology](http://storm.apache.org/releases/2.2.0/images/topology.png)
...
The object containing the processing logic implements the [IRichSpout](http://storm.apache.org/releases/2.2.0/javadocs/org/apache/storm/topology/IRichSpout.html) interface for spouts and the [IRichBolt](http://storm.apache.org/releases/2.2.0/javadocs/org/apache/storm/topology/IRichBolt.html) interface for bolts.
...
The `cleanup` method is called when a Bolt is being shutdown and should cleanup any resources that were opened. There's no guarantee that this method will be called on the cluster: for example, if the machine the task is running on blows up, there's no way to invoke the method. The `cleanup` method is intended for when you run topologies in [local mode](http://storm.apache.org/releases/2.2.0/Local-mode.html) (where a Storm cluster is simulated in process), and you want to be able to run and kill many topologies without suffering any resource leaks.

The `declareOutputFields` method declares that the `ExclamationBolt` emits 1-tuples with one field called "word".

The `getComponentConfiguration` method allows you to configure various aspects of how this component runs. This is a more advanced topic that is explained further on [Configuration](http://storm.apache.org/releases/2.2.0/Configuration.html).
...
Storm has two modes of operation: local mode and distributed mode. In local mode, Storm executes completely in process by simulating worker nodes with threads. Local mode is useful for testing and development of topologies. You can read more about running topologies in local mode on [Local mode](http://storm.apache.org/releases/2.2.0/Local-mode.html).
...
There's a few other kinds of stream groupings. You can read more about them on [Concepts](http://storm.apache.org/releases/2.2.0/Concepts.html).
